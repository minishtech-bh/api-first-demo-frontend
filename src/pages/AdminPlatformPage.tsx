...
import { useAdminPlatformPointsApiList } from '../api/admin-platform'

export default function AdminPlatformPage() {
  const [page, setPage] = useState(0)
  const { data, isLoading, isError } = useAdminPlatformPointsApiList({ page, size: 10 })
  ...
  return (
    <div>
      <h2 style={{ fontSize: 16, marginBottom: 12 }}>임직원 포인트 목록 (플랫폼)</h2>
      <table style={tableStyle}>
        <thead>
          <tr>
            {['ID', '이름', '직급', '제휴사', '잔액', '상태', '만료일'].map((h) => (
        ...
        <tbody>
          {rows.length === 0 ? (
            <tr><td colSpan={7} style={{ ...tdStyle, textAlign: 'center' }}>데이터 없음</td></tr>
          ) : (
            rows.map((r) => (
              <tr key={r.employeeInfoId}>
                ...
                <td style={tdStyle}>{r.partnerName ?? '-'}</td>
